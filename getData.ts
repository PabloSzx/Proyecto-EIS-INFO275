...
export async function getData() {
  const [
    Internacional,
    CasosConfirmadosNacionales,
    PCRRealizadosNacionales,
    FallecidosNacionales,
  ] = await Promise.all([OWIDData, Producto6Data, Producto7Data, Producto14Data]);
  ...
  const registroCasosConfirmadosAcumuladosNacionales = sortBy(
  ...
  const registroNuevosCasosConfirmadosNacionales = sortBy(
    registroCasosConfirmadosAcumuladosNacionales.map((value, index) => {
      const previousValue = registroCasosConfirmadosAcumuladosNacionales[index - 1];
      if (previousValue?.codigo_comuna === value.codigo_comuna) {
        const cantidad = Math.max(value.cantidad - previousValue.cantidad, 0);
  ...
  xslx.utils.book_append_sheet(
    wb,
    xslx.utils.json_to_sheet(registroNuevosCasosConfirmadosNacionales),
    "Nuevos Casos Nacional"
  );

  const registroNuevosPCRRealizadosNacionales = sortBy(
  ...
  xslx.utils.book_append_sheet(
    wb,
    xslx.utils.json_to_sheet(registroNuevosPCRRealizadosNacionales),
    "Nuevos PCR Nacional"
  );
  ...
  const registroNuevosFallecidosNacionales = registroFallecidosAcumulativosNacionales.map(
    (value, index) => {
  ...
  xslx.utils.book_append_sheet(
    wb,
    xslx.utils.json_to_sheet(registroNuevosFallecidosNacionales),
    "Nuevos Fallecidos Nacional"
  );
  ...
  const registroNuevosFallecidosInternacionales = sortBy(
  ...
  xslx.utils.book_append_sheet(
    wb,
    xslx.utils.json_to_sheet(registroNuevosFallecidosInternacionales, {
      cellDates: true,
    }),
    "Nuevos Fallecidos Internacional"
  );

  const registroNuevosPCRRealizadosInternacionales = sortBy(
  ...
  xslx.utils.book_append_sheet(
    wb,
    xslx.utils.json_to_sheet(registroNuevosPCRRealizadosInternacionales),
    "Nuevos PCR Internacional"
  );

  const registroNuevosCasosConfirmadosInternacionales = sortBy(
  ...
  xslx.utils.book_append_sheet(
    wb,
    xslx.utils.json_to_sheet(registroNuevosCasosConfirmadosInternacionales),
  ...
  return {
    ...
    registroNuevosPCRRealizadosNacionales,
    registroNuevosFallecidosNacionales,
    continentes,
    paises,
    registroNuevosFallecidosInternacionales,
    registroNuevosPCRRealizadosInternacionales,
